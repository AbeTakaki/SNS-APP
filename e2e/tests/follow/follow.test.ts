...
import { testCreateXweet, testCreateUser, testLogin, testCreateFollow } from '../utils';
...
test.beforeEach(async ({ request }) => {
  ...
  for(let i=0;i<5;i++){
    xweets[i]=await testCreateXweet(users[i].id,`I am ${users[i].display_name}`);
  }
...
test('非ログイン時、全ユーザのXweetが表示される', async ({ page }) => {
  await page.goto(`${process.env.TEST_FRONT_URL}/xweet`);
  for(let i=0;i<5;i++){
    await expect(page.locator('#xweet-content')).toContainText([`I am ${users[i].display_name}`]);
  }
});
...
test('ログイン時、自分及びフォロー中のユーザのXweetのみが表示される', async ({ page }) => {
  await testLogin(page,users[2].email,'password');
  await page.goto(`${process.env.TEST_FRONT_URL}/xweet`);
  for(let i=0;i<2;i++){
    await expect(page.locator('#xweet-content')).not.toContainText([`I am ${users[i].display_name}`]);
  }
  for(let i=2;i<5;i++){
    await expect(page.locator('#xweet-content')).toContainText([`I am ${users[i].display_name}`]);
  }
